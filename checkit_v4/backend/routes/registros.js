...
router.get('/stats/hoy', (req, res) => {
  ...
  const entradasHoy =
    movimientosHoy.find(m => m.nombre_movimiento === 'Entrada')?.total || 0;

  const salidasHoy =
    movimientosHoy.find(m => m.nombre_movimiento === 'Salida')?.total || 0;

  // Equipos actualmente adentro
