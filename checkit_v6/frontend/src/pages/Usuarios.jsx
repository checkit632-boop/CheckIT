...
export default function Usuarios() {
  ...
  const filtered = useMemo(() => {
    ...
    return usuarios.filter((u) =>
      [u.nombre, u.apellidos, u.usuario, u.correo, u.rol, u.numero_documento]
        .join(' ')
        .toLowerCase()
        .includes(s)
    );
  ...
  const handleSave = async (e) => {
    ...
    if (!form.correo || !CORREO_REGEX.test(form.correo)) {
      showToast(
        'El correo es obligatorio y debe ser real: ahí llegará el PIN de inicio de sesión',
        'error'
      );
      return;
    }
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Administra los usuarios del sistema (según tu rol: Administradores y/o Operadores)
        </p>
      </div>
          ...
          <tbody className="divide-y divide-gray-50 dark:divide-gray-700 text-gray-800 dark:text-gray-200">
            {filtered.map((u) => (
              <tr key={u.id_usuario} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="px-5 py-3">
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-semibold ${u.rol === 'Super Administrador' ? 'bg-brand-800 text-white' : u.rol === 'Administrador' ? 'bg-gray-800 text-white' : 'bg-blue-100 text-blue-800'}`}
                  >
                    {u.rol}
                  </span>
                </td>
                <td className="px-5 py-3">
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-semibold ${u.estado ? 'bg-brand-100 dark:bg-brand-900/40 text-brand-800 dark:text-brand-300' : 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-300'}`}
                  >
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, id_tipo_documento: Number(e.target.value) }))
                }
                required
              >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-1">
                Debe ser un correo real y al que la persona tenga acceso: ahí le llegará el PIN de 6
                dígitos para iniciar sesión.
              </p>
            </div>
            <div>
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-semibold text-sm py-2.5 rounded-xl"
            >
              Cancelar
            </button>
            <button
