...
export default function Resumen() {
  ...
  const ejecutarCSV = (data, filename, empresaNombre) => {
    const headers = ['Fecha', 'Hora Ingreso', 'Hora Salida', 'Serial', 'Responsable', 'Equipo', 'Registrado por'];
    ...
    const pie = `"CheckIT — ${empresaNombre || 'TECNOSOFT'}"`;
    const csvContent = '\uFEFF' + [headers.join(','), ...rows.map((e) => e.join(',')), '', pie].join('\n');
  ...
  const procesarExportacion = async () => {
    try {
      ...
      autoTable(doc, {
        head: [['FECHA', 'HORA INGRESO', 'HORA SALIDA', 'SERIAL', 'RESPONSABLE', 'EQUIPO', 'REGISTRADO POR']],
        body: tableRows,
        startY: tableStartY,
        theme: 'grid',
        headStyles: {
        ...
        didDrawPage: (data) => {
          ...
          if (logoFooterDataUrl) {
            try { doc.addImage(logoFooterDataUrl, 'PNG', data.settings.margin.left, footerY - 6, 8, 8); } catch (e) { /* ignore */ }
          }
          ...
          doc.setFont('helvetica', 'bold');
          doc.text(`CheckIT · ${EMPRESA_FIJA}`, pageWidth - data.settings.margin.right, footerY, { align: 'right' });
        },
      });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-2rem)] space-y-4 pb-2 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Resumen General</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Reportes y estadísticas del sistema</p>
        </div>
      ...
      <div className="flex-none grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
        <div className="md:col-span-2 bg-surface dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-5 flex flex-col justify-between">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block mb-1">Búsqueda rápida:</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block mb-1">Filtrar por categoría:</label>
                <select
                  value={filterType}
              ...
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block mb-1">Seleccionar opción:</label>
                {filterType === 'todos' ? (
                  <input
                    type="text"
                    disabled
        ...
        <div className="bg-surface dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col justify-between">
          ...
          <div className="p-4 space-y-2.5">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="px-2.5 py-0.5 rounded-full bg-brand-100 dark:bg-brand-900/40 text-brand-800 dark:text-brand-300 font-bold">{entradas}</span>
            </div>
            <div className="flex items-center justify-between text-xs">
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-surface dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="flex-none px-5 py-3 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <div className="font-bold text-gray-900 dark:text-gray-100 text-sm">Historial de Movimientos</div>
          <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">{registrosFiltrados.length} registros encontrados</span>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-700 text-xs">
              {registrosFiltrados.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center text-gray-400 dark:text-gray-500 py-10">No se encontraron movimientos registrados</td>
                </tr>
              ) : (
                registrosFiltrados.map((m) => (
                  <tr key={m.cicloKey} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors">
                    <td className="px-5 py-3">
                      <div className="font-bold text-gray-900 dark:text-gray-100">{m.persona}</div>
                      <div className="text-[10px] text-gray-400 dark:text-gray-500 font-mono">{m.documento}</div>
                    </td>
                    <td className="px-5 py-3 font-medium text-gray-800 dark:text-gray-100">{m.equipo}</td>
                    <td className="px-5 py-3 font-mono font-bold text-brand-600 dark:text-brand-400">{m.serial}</td>
                    <td className="px-5 py-3 text-center">
                      {m.entrada ? (
                        <div className="inline-flex flex-col items-center">
                          <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-brand-100 dark:bg-brand-900/40 text-brand-800 dark:text-brand-300 leading-none mb-0.5">Entrada</span>
                          <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">{fmt(m.entrada)}</span>
                        </div>
                    ...
                    <td className="px-5 py-3 text-center">
                      {m.salida ? (
                        <div className="inline-flex flex-col items-center">
                          ...
                          <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">{fmt(m.salida)}</span>
                        </div>
                      ) : <span className="text-gray-300 dark:text-gray-600 font-mono">—</span>}
                    </td>
                    <td className="px-5 py-3 font-medium text-gray-700 dark:text-gray-300">{m.registrado_por}</td>
                    <td className="px-5 py-3 text-gray-500 dark:text-gray-400 max-w-[180px] truncate" title={m.observaciones}>{m.observaciones || '—'}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-surface dark:bg-gray-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-gray-100 dark:border-gray-700 max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-700">
              ...
              <button onClick={() => setExportModalOpen(false)} className="text-gray-400 dark:text-gray-500 hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="space-y-4 my-4">
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block mb-1">Rango / Modo de Fecha:</label>
                <select
                  value={timeFilter}
              ...
              {timeFilter === 'rango' && (
                <div className="grid grid-cols-2 gap-3 bg-brand-50/50 dark:bg-brand-900/20 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
                  <div>
                    <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Fecha Inicio:</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Fecha Fin:</label>
                    <input
                      type="date"
                      value={customEndDate}
              ...
              {timeFilter === 'dia_especifico' && (
                <div className="bg-brand-50/50 dark:bg-brand-900/20 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
                  <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Seleccionar Día:</label>
                  <input
                    type="date"
                    value={singleDate}
              ...
              {timeFilter === 'mensual' && (
                <div className="bg-brand-50/50 dark:bg-brand-900/20 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
                  <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Seleccionar Mes y Año:</label>
                  <input
                    type="month"
                    value={selectedMonth}
                    ...
                    className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-brand-500 bg-surface dark:bg-gray-800"
                  />
                  <p className="text-[10px] text-brand-700 mt-1 font-medium">✓ Abarca desde el día 1 hasta el cierre del mes seleccionado.</p>
                </div>
              )}

              {timeFilter === 'anual' && (
                <div className="bg-brand-50/50 dark:bg-brand-900/20 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
                  <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Año a Consultar:</label>
                  <input
                    type="number"
                    min="2020"
                    max="2030"
              ...
              {timeFilter === 'ultimos_x' && (
                <div className="grid grid-cols-2 gap-3 bg-brand-50/50 dark:bg-brand-900/20 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
                  <div>
                    <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Cantidad:</label>
                    <input
                      type="number"
                      min="1"
                      value={timeAmount}
                  ...
                  <div>
                    <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Unidad:</label>
                    <select
                      value={timeUnit}
              ...
              <div className="pt-3 border-t border-gray-100 dark:border-gray-700">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-gray-900 dark:text-gray-100 flex items-center gap-1.5">
                    <Settings size={14} className="text-brand-600 dark:text-brand-400" /> Personalización de Empresa / Marca
                  </span>
                  <input
                    type="checkbox"
                ...
                {useCustomBrand && (
                  <div className="space-y-3 bg-slate-50 dark:bg-gray-900 p-3.5 rounded-2xl border border-slate-200 dark:border-gray-700">
                    <div>
                      <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Nombre de la Empresa:</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-[11px] font-bold text-gray-700 dark:text-gray-300 block mb-1">Cargar Logotipo:</label>
                      <div className="flex items-center gap-2">
                        ...
                        {logoUrl && <span className="text-[10px] text-brand-600 dark:text-brand-400 font-bold">✓ Cargado</span>}
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div>
                <label className="text-xs font-bold text-gray-700 dark:text-gray-300 block mb-1">Pie de página (Copyright / Leyenda):</label>
                <input
                  type="text"
                  value={footerNote}
