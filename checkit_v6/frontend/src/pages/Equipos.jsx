...
import { Search, Plus, Pencil, Trash2, QrCode, Power, PowerOff } from 'lucide-react';
...
const inputCls =
  'mt-1 w-full border border-gray-200 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500';
...
export default function Equipos() {
  ...
  const filtered = useMemo(() => {
    ...
    return equipos.filter((e) =>
      [e.serial, e.nombre_marca, e.modelo, e.persona_nombres, e.persona_apellidos]
        .join(' ')
        .toLowerCase()
        .includes(s)
    );
  ...
  const filteredPersonas = useMemo(() => {
    ...
    return personas
      .filter((p) =>
        [p.nombres, p.apellidos, p.numero_documento].join(' ').toLowerCase().includes(s)
      )
      .slice(0, 8);
    ...
    try {
      ...
      if (nuevaPersona) {
        if (!personaForm.numero_documento || !personaForm.nombres || !personaForm.apellidos) {
          showToast('Completa los datos de la persona', 'error');
          return;
        }
      ...
      if (!idPersona || !form.id_marca || !form.serial || !form.id_estado) {
        showToast('Marca, serial, condición y responsable son obligatorios', 'error');
        return;
      }
  ...
  // Inactivar/activar (solo Super Administrador) — para equipos dados de
  // baja o retirados. No borra el historial de movimientos del equipo.
  const handleToggleActivo = async (eq) => {
    const accion = eq.activo ? 'inactivar' : 'activar';
    if (!confirm(`¿Seguro que quieres ${accion} el equipo ${eq.serial}?`)) return;
    try {
      await api.patch(`/equipos/${eq.id_equipo}/estado`, { activo: !eq.activo });
      showToast(`Equipo ${eq.activo ? 'inactivado' : 'activado'} correctamente`);
      loadAll();
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Gestiona el inventario de computadores del sistema
          </p>
        </div>
        <div className="bg-brand-50 dark:bg-brand-900/30 border border-brand-200 dark:border-brand-800 text-brand-800 dark:text-brand-200 rounded-xl px-4 py-2 text-sm font-bold">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button
          onClick={openNew}
          className="ml-auto flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors"
        >
      ...
      <div className="bg-surface dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700 font-bold text-gray-900 dark:text-gray-100">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-gray-500 text-xs uppercase sticky top-0">
              <tr>
                ...
                <th className="text-left px-5 py-2.5">Condición</th>
            ...
            <tbody className="divide-y divide-gray-50">
              ...
              {filtered.map((e) => (
                <tr
                  key={e.id_equipo}
                  className={`hover:bg-gray-50 ${!e.activo ? 'opacity-60' : ''}`}
                >
                  <td className="px-5 py-3 font-mono-num font-semibold">{e.serial}</td>
                  <td className="px-5 py-3">
                    <strong>{e.nombre_marca}</strong> {e.modelo}
                  </td>
                  <td className="px-5 py-3">
                    {e.persona_nombres} {e.persona_apellidos}
                  </td>
                  <td className="px-5 py-3">
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-semibold ${estadoBadge(e.nombre_estado)}`}
                    >
                      {e.nombre_estado}
                    </span>
                  </td>
                  <td className="px-5 py-3">
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-semibold ${e.activo ? 'bg-brand-100 dark:bg-brand-900/40 text-brand-800 dark:text-brand-300' : 'bg-red-100 dark:bg-red-950/40 text-red-700 dark:text-red-300'}`}
                    >
                  ...
                  <td className="px-5 py-3">
                    <button
                      onClick={() => setQrEquipo(e)}
                      className="flex items-center gap-1 text-brand-700 bg-brand-100 hover:bg-brand-200 px-2.5 py-1 rounded-lg text-xs font-semibold"
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        {isSuperAdmin && (
                          <button
                            ...
                            title={e.activo ? 'Inactivar equipo' : 'Activar equipo'}
                            className={`p-1.5 rounded-lg hover:bg-gray-100 ${e.activo ? 'text-amber-600' : 'text-brand-600'}`}
                          >
                            {e.activo ? <PowerOff size={15} /> : <Power size={15} />}
                          </button>
                        )}
                        <button
        ...
        <form onSubmit={handleSave} className="space-y-4">
                    ...
                    {filteredPersonas.map((p) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setPersonaSearch(`${p.nombres} ${p.apellidos} — ${p.numero_documento}`);
                        }}
                      >
                        {p.nombres} {p.apellidos}{' '}
                        <span className="text-gray-400">— {p.numero_documento}</span>
                      </div>
                    ))}
              ...
              <div className="border border-gray-200 rounded-xl p-3 mt-1 space-y-3 bg-gray-50">
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    onChange={(e) => setPersonaForm((f) => ({ ...f, nombres: e.target.value }))}
                  />
                  <input
                    className={inputCls + ' mt-0'}
                    placeholder="Apellidos"
                    value={personaForm.apellidos}
                    onChange={(e) => setPersonaForm((f) => ({ ...f, apellidos: e.target.value }))}
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    onChange={(e) =>
                      setPersonaForm((f) => ({ ...f, numero_documento: e.target.value }))
                    }
                  />
                  <input
                    className={inputCls + ' mt-0'}
                    ...
                    onChange={(e) => setPersonaForm((f) => ({ ...f, celular: e.target.value }))}
                  />
                </div>
                <input
                  className={inputCls + ' mt-0'}
                  ...
                  onChange={(e) => setPersonaForm((f) => ({ ...f, correo: e.target.value }))}
                />
                <button
                  type="button"
                  onClick={() => setNuevaPersona(false)}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Se mostrará en cada entrada/salida del equipo, sin tener que volver a escribirla"
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-semibold text-sm py-2.5 rounded-xl"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="flex-1 bg-brand-600 hover:bg-brand-700 text-white font-semibold text-sm py-2.5 rounded-xl transition-colors"
            >
              Guardar Equipo
            </button>
          </div>
