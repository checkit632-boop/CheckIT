...
import {
  LayoutGrid, Laptop, ArrowLeftRight, FileBarChart2, Users, QrCode, LogOut,
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 shrink-0 bg-gradient-to-b from-brand-950 via-gray-900 to-brand-900 text-white flex flex-col h-screen sticky top-0">
      <div className="px-5 py-6 border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-bold text-lg leading-tight">CheckIT</div>
            <div className="text-xs text-gray-400">Sistema de Control</div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4 border-t border-white/10">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="min-w-0">
            <div className="text-sm font-semibold truncate">{user?.nombre} {user?.apellidos}</div>
            <div className="text-xs text-brand-400 truncate">{user?.rol}</div>
          </div>
        </div>
        <button
